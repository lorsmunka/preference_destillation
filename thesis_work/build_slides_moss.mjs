...
// Forest & Moss palette
// Primary: forest green (2C5F2D) — dominant, titles and dark backgrounds
// Secondary: moss green (97BC62) — supporting tone for accents on dark BG
// Accent: warm amber (C97B1F) — sharp warm contrast on white for callouts
const NAVY = "2C5F2D";   // forest green (var name kept for minimal diff)
const ICE = "C8D9A6";    // light moss/sage tint, readable on dark BG
const WHITE = "FFFFFF";
const BODY = "2A2A2A";
const MUTED = "6B6B6B";
const ACCENT = "C97B1F"; // warm amber, complements forest, pops on white
...
function subtitle(slide, text) {
  slide.addText(text, {
    ...
    fontSize: 14, italic: true, fontFace: FB, color: MUTED,
...
add(() => {
  ...
  s.addText("Témavezető: Szénási Sándor, egyetemi tanár", {
    x: 0.8, y: 5.0, w: W - 1.6, h: 0.4,
    fontSize: 16, fontFace: FB, color: ICE, align: "left"
  });
  s.addText("TDK · 2026", {
    x: 0.8, y: 6.6, w: W - 1.6, h: 0.4,
    fontSize: 13, italic: true, fontFace: FB, color: ICE, align: "left"
  });
});
...
add(() => {
  ...
  s.addText("Rövid szavankénti magyarázat", {
    ...
    fontSize: 16, italic: true, fontFace: FB, color: MUTED, align: "center"
  });
});
...
add(() => {
  ...
  s.addText("Pl. benzin, dízel, kerozin külön frakciókba.", {
    x: leftX, y: colY + 2.3, w: colW, h: 1.0,
    fontSize: 16, italic: true, fontFace: FB, color: MUTED, valign: "top"
  });
  ...
  s.addText("Megjegyzés: LLM kontextusban a desztilláció tömörítést is jelenthet.", {
    x: rightX, y: colY + 2.3, w: colW, h: 1.2,
    fontSize: 16, italic: true, fontFace: FB, color: ACCENT, valign: "top"
  });
...
add(() => {
  ...
  s.addText("Bemenet:", {
    ...
    fontSize: 14, bold: true, fontFace: FB, color: MUTED
  });
...
Solution: ?;`,
    {
      x: 0.7, y: 2.25, w: W - 1.4, h: 2.4,
      fontSize: 16, fontFace: FM, color: BODY, valign: "top"
    }
  );
  s.addText("Kimenet:", {
...
Solution: 42;`,
    {
      x: 0.7, y: 5.2, w: W - 1.4, h: 1.8,
      fontSize: 16, fontFace: FM, color: NAVY, valign: "top"
    }
  );
});
...
add(() => {
  ...
  const head = { bold: true, color: WHITE, fill: { color: NAVY }, align: "center", valign: "middle", fontSize: 14, fontFace: FB };
  const cell = { color: BODY, align: "center", valign: "middle", fontSize: 16, fontFace: FB };
  const rows = [
    [
...
add(() => {
  ...
  s.addText("Vs. teljes Gemma kimeneti vocab: 262 144 token", {
    ...
    fontSize: 14, italic: true, fontFace: FB, color: MUTED, align: "center"
  });
});
...
await pres.writeFile({ fileName: path.join(__dirname, "slides_moss.pptx") });
console.log("Wrote slides_moss.pptx (" + slides.length + " slides)");
